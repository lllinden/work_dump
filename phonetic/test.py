...
def getIPA_CMU(userInputRaw):
    userInput =[]
    userInputRaw = re.sub("\"", "", userInputRaw)
    if ' ' in userInputRaw:
...
# def get_finalPrint(IPA_CMU):
    # final_IPA = get_IPA(IPA_CMU)
def get_kk(input):
    raw_phonetic_str=get_IPA(input)
    phonetic_list = [ x for x in {}.fromkeys(raw_phonetic_str.split(',')).keys() if x != '']
    result_str = ''
    for x in phonetic_list:
       result_str += x + "|"
    return result_str

def get_kk_json(input):
    raw_phonetic_str=get_IPA(input)
    phonetic_list = [ x for x in {}.fromkeys(raw_phonetic_str.split(',')).keys() if x != '']
    result_str = ''
    for x in phonetic_list:
        result_str += x + "|"
    return "{\"word\":\""+re.sub("\"", "", input).strip()+"\",\"phonetic\":\""+result_str.strip("|")+"\"}"



if __name__ == '__main__':
    result = get_kk_json(sys.argv[1])
    print result
